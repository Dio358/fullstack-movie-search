...
async def test_users(db):
    ...
    try:
        ...
        assert Hasher.verify_password(
            password=password,
            hashed_password=user[0]["password"].encode('utf-8')
        )

        uid_from_table = await user_table.get_user_id(username=username, password=password)
...
async def test_favorites(db):
    ...
    try:
        ...
        assert any(f["movie_id"] == movie_id and f["user_id"] == user_id for f in favorites)
