...
const List: React.FC<Props> = ({ items, maxHeight = "300px", length = Number.POSITIVE_INFINITY, onClick, onHover = null }) => {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  
  useEffect(() => {
    if (onHover) {
      onHover(hoveredIndex? (items?.[hoveredIndex]? items?.[hoveredIndex] : null)  : null);
    }
